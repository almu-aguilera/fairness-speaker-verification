...
def computeTrain(train):
    ...
    for idx in df_temp.index:
        for smp in range(0,df_temp.loc[idx,'n_sample'],1):
            audio=df_temp.loc[idx,'id_user']+'/'+"audio"+f'{smp:05}'+".wav"
...
def addRowToTest(speaker_1,speakers_2,triple_ts,idx,index,i):
    ...
    audio_1=speaker_1.loc[idx,'id_user']+'/'+"audio"+f'{i:05}'+".wav"
    rnd_smp=random.randint(0,speakers_2.loc[rnd_usr,'n_sample']-1)
    audio_2=speakers_2.loc[rnd_usr,'id_user']+'/'+"audio"+f'{rnd_smp:05}'+".wav"
...
def computeTest(test):
    ...
    for idx in test.index:
        ...
        for lis in combination_list:
            for a in lis:
                audio_1=test.loc[idx,'id_user']+'/'+"audio"+f'{a[0]:05}'+".wav"
                audio_2=test.loc[idx,'id_user']+'/'+"audio"+f'{a[1]:05}'+".wav"
